...
const register = async (req, res) => {
    ...
    try {
        ...
        // 4. Save user
        const newUser = await (0, db_js_1.query)('INSERT INTO users (name, email, password) VALUES ($1, $2, $3) RETURNING id, name, email, created_at', [name.trim(), email.toLowerCase().trim(), hashedPassword]);
        const userPayload = newUser.rows[0];
        // 5. Generate JWT Token
        const token = jsonwebtoken_1.default.sign({ id: userPayload.id, email: userPayload.email, name: userPayload.name }, JWT_SECRET, { expiresIn: '7d' } // Session persists for 7 days
        );
...
const login = async (req, res) => {
    ...
    try {
        ...
        const result = await (0, db_js_1.query)('SELECT * FROM users WHERE email = $1', [email.toLowerCase().trim()]);
...
const getMe = async (req, res) => {
    ...
    try {
        const result = await (0, db_js_1.query)('SELECT id, name, email, created_at FROM users WHERE id = $1', [req.user.id]);
        if (result.rows.length === 0) {
