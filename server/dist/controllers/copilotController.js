...
exports.triggerAdaptiveRecalculate = exports.getReadiness = exports.getGPS = exports.updateProjectStatus = exports.getProjects = exports.updateTaskStatus = exports.getMissions = exports.getGoal = exports.setGoal = exports.recalculateJobReadiness = void 0;
...
// ==========================================
const recalculateJobReadiness = async (userId) => {
    let atsScore = 0;
    ...
    try {
        const analysisRes = await (0, db_js_1.query)(`SELECT ats_score, resume_quality_score, matched_skills, suggestions 
       FROM analyses 
       WHERE user_id = $1 
    ...
    try {
        const goal = await prisma_js_1.dbService.getCareerGoal(userId);
        if (goal) {
            const analysisRes = await (0, db_js_1.query)(`SELECT matched_skills, missing_skills 
         FROM analyses 
         WHERE user_id = $1 
         ORDER BY created_at DESC 
         LIMIT 1`, [userId]);
    ...
    let projectScore = 0;
    try {
        const projects = await prisma_js_1.dbService.getProjectRecommendations(userId);
    ...
    try {
        const goal = await prisma_js_1.dbService.getCareerGoal(userId);
        if (goal) {
            const session = await prisma_js_1.dbService.getInterviewSession(userId, goal.targetRole);
            if (session) {
    ...
    // 5. Overall Weighted
    const overallScore = Math.round(resumeScore * 0.25 +
        skillScore * 0.35 +
        projectScore * 0.25 +
        interviewScore * 0.15);
    ...
    const pendingProjects = (await prisma_js_1.dbService.getProjectRecommendations(userId)).filter(p => p.status !== 'Completed');
    ...
    const finalRecord = await prisma_js_1.dbService.saveJobReadinessScore(userId, {
        overallScore,
        resumeScore,
        skillScore,
        projectScore,
    ...
    await prisma_js_1.dbService.calculateUserProgress(userId);
    return finalRecord;
};
exports.recalculateJobReadiness = recalculateJobReadiness;
// ==========================================
...
const setGoal = async (req, res) => {
    ...
    try {
        // 1. Save Goal Info
        const goal = await prisma_js_1.dbService.saveCareerGoal(req.user.id, {
            targetRole,
        ...
        let resumeData;
        const analysisRes = await (0, db_js_1.query)(`SELECT ats_score, resume_quality_score, matched_skills, missing_skills 
       FROM analyses 
       WHERE user_id = $1 
       ORDER BY created_at DESC 
       LIMIT 1`, [req.user.id]);
        ...
        const aiCopilotResponse = await (0, gemini_js_1.generateCareerCopilotRoadmap)(targetRole, Number(timelineMonths), currentSkillLevel, Number(hoursPerDay), preferredLearningStyle, Array.isArray(currentSkills) ? currentSkills : [], resumeData);
        // 4. Save generated Roadmap & Weekly Missions
        const roadmap = await prisma_js_1.dbService.saveRoadmap(goal.id, req.user.id, aiCopilotResponse.roadmapTitle, aiCopilotResponse.roadmapDescription, aiCopilotResponse.modules);
        // 5. Save Project recommendations
        await prisma_js_1.dbService.saveProjectRecommendations(goal.id, aiCopilotResponse.projectRecommendations);
        // 6. Recalculate Initial Readiness Score
        await (0, exports.recalculateJobReadiness)(req.user.id);
...
const getGoal = async (req, res) => {
    ...
    try {
        const goal = await prisma_js_1.dbService.getCareerGoal(req.user.id);
        if (!goal) {
            return res.status(200).json({ goal: null, roadmap: null });
        }
        const roadmap = await prisma_js_1.dbService.getRoadmap(req.user.id);
...
const getMissions = async (req, res) => {
    ...
    try {
        const missions = await prisma_js_1.dbService.getWeeklyMissions(req.user.id);
...
const updateTaskStatus = async (req, res) => {
    ...
    try {
        const updatedMission = await prisma_js_1.dbService.updateMissionTaskStatus(req.user.id, Number(missionId), taskId, status);
...
const getProjects = async (req, res) => {
    ...
    try {
        const projects = await prisma_js_1.dbService.getProjectRecommendations(req.user.id);
...
const updateProjectStatus = async (req, res) => {
    ...
    try {
        const updatedProject = await prisma_js_1.dbService.updateProjectStatus(req.user.id, Number(projectId), status);
...
const getGPS = async (req, res) => {
    ...
    try {
        ...
        const scoresHistory = await prisma_js_1.dbService.getReadinessHistory(req.user.id);
        ...
        const roadmap = await prisma_js_1.dbService.getRoadmap(req.user.id);
        const weeklyMissions = await prisma_js_1.dbService.getWeeklyMissions(req.user.id);
        ...
        if (incompleteMissions.length > 0) {
            ...
            const pendingTask = activeMission.tasks.find(t => t.status !== 'Completed');
 ...
 */
const triggerAdaptiveRecalculate = async (req, res) => {
    if (!req.user) {
        return res.status(401).json({ error: 'Unauthorized', message: 'User session not found.' });
    }
    try {
       ...
       LIMIT 1`, [req.user.id]);
        ...
        const aiCopilotResponse = await (0, gemini_js_1.generateCareerCopilotRoadmap)(goal.targetRole, goal.timelineMonths, goal.currentSkillLevel, goal.hoursPerDay, goal.preferredLearningStyle, [], resumeData);
...
exports.triggerAdaptiveRecalculate = triggerAdaptiveRecalculate;
